...
function processPlayers(allPlayerStats) {
    ...
    const allPlayerStatLines = allPlayerStats.split(/\r\n|\n/);
    ...
    for (var playerStats of allPlayerStatLines) {
        ...
        const per = parseFloat(stats[9]);

        // Add per value to player's array (the next quarter)
        playerMap[playerName].push(per);
    }
...
function displayPlayerBench() {
    ...
    const playersOnBench = document.getElementById('playersOnBench');

    // For each player, create a button 
    for (let playerName in playerMap) {
        ...
        playersOnBench.appendChild(newPlayer);
    }
...
function displayPlayerCards() {
    ...
    const displayPlayerCard = document.getElementById('playerCards');
    ...
    for (let playerName in playerMap) {
        ...
        statCard.id = playerName + '_card';

        // Set the style class name
        statCard.className = 'playerCard tooltip';

        // Add the player's name to the div.
        const newPlayerNameTooltip = document.createElement('SPAN');
        newPlayerNameTooltip.className = 'tooltiptext';
        newPlayerNameTooltip.innerText = playerName;
        statCard.appendChild(newPlayerNameTooltip);
        ...
        statCard.appendChild(playerImage);

        // Add the player's PER to the div.
        const newPlayerPER = document.createElement('P');

        // Set the style for the number
        newPlayerPER.className = 'perCard';

        // Set the text for the PER
        newPlayerPER.innerText = 'PER: ' + playerMap[playerName][currentQuarter].toPrecision(4);

        // Add the PER
        statCard.appendChild(newPlayerPER);

        // Add the player stat card to the game.
        displayPlayerCard.appendChild(statCard);
    }
}
...
function movePlayer() {
    ...
    if (parentDiv.id === 'playersOnBench') {
        ...
        if (playersOnCourt === maxPlayersOnCourt) {
            alert('There is already ' + playersOnCourt + ' players on the court - which is the maximum');
            return;
        } else {
            ...
            playersOnCourt++;
            quarterPER += playerMap[this.id][currentQuarter];
...
function updateCardsInGame() {
    ...
    // a specific quarter.
    for (let playerName in playerMap) {
        document.getElementById(playerName + '_card').children[2].innerText = 'PER: ' + playerMap[playerName][currentQuarter].toPrecision(4);
    }
    ...
    const currentPlayers = document.getElementById('playersOnCourt').children;

    // Loop through each of the players currently on the court.
    for (let i = 0; i < currentPlayers.length; i++) {
        // Get the name of the player
        const playerName = currentPlayers[i].id;

        // Get the PER for the player
        const playerPER = playerMap[playerName][currentQuarter];
...
function endGame() {
    ...
    const averagePER = totalAvePER / numQuarters;
